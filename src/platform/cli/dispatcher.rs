...
pub async fn dispatch(command: CliCommand, format: OutputFormat) -> Result<(), String> {
    match command {
        ...
        CliCommand::Contract(args) => {
            // Seluruh logika kontrak berada di `contract::cli` (Contract SDK
            // bridge): parsing argumen, keystore, simulasi, dan broadcast.
            crate::platform::contract::cli::handle_contract_subcommand(&args, format)
        }
